...
class CustomImageDataset(Dataset):
    def __init__(self, root, train=True, transform=None):
        self.data_set = datasets.CIFAR10(root=root, train=train, download=True)
    ...
    def __getitem__(self, index):
        image, label = self.data_set[index]
        if self.transform:
    ...
    def variance(self):
        return np.var(self.data_set.data / 255.0)
...
training_data = CustomImageDataset(root="data", train=True, transform=transform)
validation_data = CustomImageDataset(root="data", train=False, transform=transform)
...
# %% Encoder/Decoder base on ResNet
class VectorQuantizerEMA(nn.Module):
    def __init__(
        self, num_embeddings, embedding_dim, commitment_cost, decay, epsilon=1e-5
    ):
